...
A cena é a oficina de Babbage. O jogador anda com WASD/setas ou mouse e precisa
...
"ligar a máquina" (babbage_lovelace.py). Apos resolve-lo o há uma transeção (fade) que leva o personagem para uma sala separada, só
com a máquina do tempo — ele entra pela lateral esquerda e anda sozinho
(o jogador não consegue controlar) até perto dela; ao chegar, o jogador recupera o
controle e só precisa clicar nela para concluir a fase.

`run()` é a função a principal deste arquivo e reutiliza a mesma janela/clock
do jogo principal (quem chama já criou a janela, esta função só "toma
emprestado" a tela por um tempo). Ela Devolve `True` se o jogador concluiu a
fase (clicou na máquina do tempo), ou `False` se saiu antes apertando ESC.

Resumo de quem chama quem, de cima pra baixo:
    ...
    ├── ada_chatbot.AdaChat --> retrato clicável da Ada + chat com a IA (que só aparece na oficina)
...
import math  # importa todas as bibliotecas necessárias primeiro
import os
...
# para funcionar não importa de onde o jogo seja executado.
ASSETS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")

# ---------------------------------------------------------------------------
# Configurações: cores que vão aparecer na interface 
# ---------------------------------------------------------------------------
# Dourado para os títulos, bege para instruções, verde-oliva para status ou sucesso.
GOLD = (212, 168, 67) #titulos
CREAM = (232, 212, 176) #instruções
OLIVE = (138, 155, 110) #status/sucesso
RED = (255, 70, 70) 
WHITE = (225, 230, 235) #outras msgs (controles- instruções para o jogador)
GREEN = (0, 255, 140)  # usado só em marcadores/efeitos visuais, não no texto em si

# ---------------------------------------------------------------------------
# Configurações para os personagem e colisão
# ---------------------------------------------------------------------------
# PLAYER_RADIUS não é um tamanho visual, ou seja, o sprite pode ser maior/menor) --
...
# o personagem nunca chegaria a encostar no pixel do alvo

CLICK_ARRIVE_DIST = 4

# ---------------------------------------------------------------------------
# Configurações para os textos fixos da interface 
# ---------------------------------------------------------------------------
TITLE = "FASE 2"
SUBTITLE = "AS MÁQUINAS MECÂNICAS  —  1800-1840"
HINT = "WASD / SETAS andam, CLIQUE interage, ESC volta ao mapa"  #aparece o tempo td na fase no 1° cenário
MACHINE_HINT = "Entre na máquina do tempo para ir para a próxima fase!" #aparece o tempo todo no segundo cenario apos o personagem resovler os puzzles

# ---------------------------------------------------------------------------
# Configurações na tela de introdução que aparece uma única vez, antes da cena
...
# ---------------------------------------------------------------------------
# Configurações para engrenagens que o jogador deverá colecionar (visual de hover/destaque)
# ---------------------------------------------------------------------------
GEAR_SPRITE_SIZE = 24
...
SHADOW_ALPHA = 140  # opacidade da sombra em volta sob os objetos soltos no chão

# ---------------------------------------------------------------------------
# Configurações para os objetos colecionáveis (baú, pote, caixote)
# ---------------------------------------------------------------------------
# Os 3 objetos com estado fechado/aberto que escondem uma engrenagem cada um.
# Cada dicionário aqui descreve UM ÚNICO objeto: onde fica, quais imagens usar
# (fechado/aberto) e como desenhá-lo. Guardar tudo assim, numa lista de
# dicionários para evitar ter que copiar/colar o mesmo bloco de código 3 vezes -->o loop lá em run() percorre essa lista e trata os 3 objetos de formas iguais.
# "fit" diz como a imagem é escalada: ("h", altura) deixa a altura fixa
# (objetos "verticais"), ("w", largura) deixa td a largura fixa.
# "shadow" (largura, altura) desenha uma sombra redonda sob o objeto, para
# deixa-lo visísel na superfície onde está; e o pote fica sobre o tampo da
# mesa (sombra pequena e sutil), já o bau e o caixote ficam soltos no chão.
CONTAINERS = [
...
# um dos desenhos de engrenagem que já existem ilustrados no papel de fundo, já que colocar como um assoalho fazia parte do chão clicável desaparecer tbm
...
# abre o puzzle, mas só depois que todas as 4 engrenagens forem coletadas.
...
# ---------------------------------------------------------------------------
# Configurações de colisão e geometria das salas
# ---------------------------------------------------------------------------
...
# de run(), que o menu repassa de acordo com o personagem selecionado. As chaves deste dicionário (ex: "avatar_parado_m") viram as
...
# módulo antes de criar a janela, o carregamento é adiado para _load_assets(), chamado no início de run(), o mesmo padrão de common.init_fonts().
BACKGROUND = None
...
def _scale_fit(img, fit, rot=0):
    ...
    # max(1, ...) evita que a imagem vire 0 pixels de largura/altura em algum caso extremo de arredondamento; pygame não aceita isso.
...
def _wrap_text(text, font, max_width):
    ...
    dada, o pygame não faz isso sozinho (renderiza a string inteira numa linha só), então essa função faz na mão o trabalho de decidir onde quebrar.
    A ideia (bem parecida com "preencher uma linha até não caber mais"):vai testando palavra por palavra se ELA CABE na linha atual junto com o que
    já tem; se couber junta e se não couber, fecha a linha atual e
    ...
    for word in words:
        ...
        if font.size(candidate)[0] <= max_width:
            ...
            # não coube mais: fecha a linha atual (se já tinha algo nela)e começa uma nova linha só com a palavra que não coube.
            if current:
...
def _load_assets():
    ''' Essa carrega (e escala) todas as imagens usadas pela Fase 2, mas só na
    ...
    # engrenagem pequena (que o jogador coleta)
    ...
    # 100% opaca, mostrada quando o mouse está por perto --> é o que dá o efeito de
    ...
    # Percorre a lista CONTAINERS que foi definida lá em cima e, pra cada objeto,
    # carrega as 2 imagens dele (fechado e aberto) e guarda cada uma já
    # escalada nos dicionários CLOSED_SPRITES/OPEN_SPRITES, indexados pela
    # "key" do objeto, e na hora de desenhar, é só
    ...
    # máquina do tempo na sala separada)
    ...
    # (TIME_MACHINE_POS): assim, se um dia eu mudar TIME_MACHINE_FIT (o
    ...
    # as 6 de uma vez só. key.replace("avatar_", "") transforma, o "avatar_parado_m" em "parado_m" --> é assim que run() depois
    # busca o frame certo em AVATAR_FRAMES (ver a escolha de sufixo mais abaixo).
...
def _draw_player(screen, pos, image, character_name, name_font):
    ...
    congelado do fade) numa posição arbitrária, mais o nome embaixo,e é
    ...
    sprite) (a mesma referência usada pela colisão (_position_allowed)), então
    o personagem sempre encosta visualmente onde a colisão realmente o para
    (ex: bem rente à bancada).
    '''
...
def _position_allowed(pos, floor_rect, table_rect=None):
    ...
    # Testa se pos tá dentro do chão andável, mas com uma margem de PLAYER_RADIUS pra dentro em cada lado, pq sem essa margem, o "ponto"
    # do personagem encostaria exatamente na borda da parede, o que pareceria estranho (ele é desenhado com um sprite bem maior que um
    # pixel).
    ...
    if table_rect is not None:
        # esse inflate aumenta o retângulo da bancada em PLAYER_RADIUS*2 (o
...
def _try_move(pos, delta, floor_rect, table_rect=None):
    ...
    Recebe a posição atual (pos), o quanto quero andar (delta, um
    ...
    # Nem o movimento completo, nem só em X, nem só em Y funcionaram o personagem fica exatamente onde estava.
    return pos, False
...
class Jogador:
    ...
    Adaptado da estrutura validada pela Natalia em outra fase: mantém os
    ...
    2. O deslocamento passa por _try_move (que desliza ao redor de
       obstáculos) em vez de um simples "clamp" (só prender dentro de um retângulo) a oficina tem uma área bloqueada
       no MEIO da sala (a bancada), não só uma borda externa, então um clamp sozinho não seria suficiente aqui.
    '''
    ...
    def __init__(self, frame_parado, frames_andando, posicao_inicial):
        ...
        self.imagem = self.frame_parado  # ele começa parado, olhando pra frente
    ...
    def mover(self, teclas, floor_rect, table_rect=None):
        ...
        """
        # Cada tecla mexe em ATÉ um eixo por vez: esquerda/direita mudam dx, cima/baixo mudam dy. Segurando duas teclas ao mesmo tempo
        # (ex: W + D) dá diagonal, pq dx e dy ficam != 0 juntos.
        dx = dy = 0
        ...
        if esta_andando:
            ...
            # depois, e isso evita que andar na diagonal (dx=1, dy=1) seja
    ...
    def mover_para(self, alvo, floor_rect, table_rect=None):
        ...
        mover(), só que o alvo é uma posição (usado só pela caminhada
        ...
        # LENGTH (comprimento) é a distância que falta pra percorrer.
        ...
        if not chegou:
            ...
            if not moved:
                ...
                # nem um pouco, ele desiste de perseguir esse alvo em vez de ficar tentando pra sempre no mesmo lugar.
                chegou = True
    ...
    def _atualizar_sprite(self, esta_andando):
        '''Decide qual frame mostrar, o parado, ou alternando entre os dois
        frames de caminhada conforme o tempo passa.

         Aí recebe esta_andando (True/False) e atualiza self.imagem. A
        ...
        '''
        if not esta_andando:
            # Parado: sempre mostra o mesmo frame, e reseta o índice da animação, assim, da próxima vez que andar, sempre começa
            # do primeiro frame de caminhada, pra ficar mais consistente.
        ...
        if agora - self.tempo_ultimo_frame >= self.INTERVALO_ANIMACAO_MS:
            # já passou tempo suficiente --> avança pro próximo frame da lista, voltando pro início quando chega no final (o `%`
            # aí faz esse "loop": depois do último frame, volta pro 0).
    ...
    def desenhar(self, tela):
        '''Desenha o personagem centralizado em self.pos com método
        auxiliar simples que não é usado no loop principal (run() usa
        a função _draw_player() do módulo, que anco como parte da
        estrutura original da classe.ra pelos PÉS em vez do
        centro, pra combinar com a colisão). Fica aqui para quando o fade de transição pra sala da máquina do tempo
        '''
...
def _run_intro(screen, clock, character_image, width, height, title_font, subtitle_font, hint_font):
    ...
    '''Tela de introdução (uma vez só, antes da cena jogável). o personagem
    ...
    jogo, False se o jogador saiu (ESC).
    Essa função tem o próprio "mini loop de jogo" (while running, com
    eventos e desenho), separado do loop principal em run() e é assim que
    ...
    # Se recebeu a imagem do personagem (vinda do menu), redimensiona pra um retrato bem grande (INTRO_PORTRAIT_HEIGHT) que é maior que o sprite
    # usado durante o jogo, pq aqui é tipo uma "cena de abertura".
    ...
    while running:
        ...
        # a janela real pode ter um tamanho diferente da tela virtual (width x height, o espaço em que todas as coordenadas desta tela
        # já são calculadas),  converte mouse/clique de volta pra cá antes de checar qualquer colisão. Mesma conta repetida em run() e em
        # babbage_lovelace.py, porque cada uma dessas telas tem seu próprio loop independente.
        ...
        # Desenha as linhas do texto (já quebradas por _wrap_text) uma embaixo da outra, centralizadas verticalmente dentro do balão,
        # text_top calcula onde a PRIMEIRA linha deve começar pra que o bloco inteiro de texto fique centralizado.
...
def _fade_transition(screen, clock, before_surface, after_surface, duration=FADE_DURATION_SECONDS):
    ...
    '''
    # Quantos frames (a 60fps) o fade inteiro deve durar 

    steps = max(1, int(duration * 60))

    # O fade tem duas metades: primeiro escurece a cena antiga até ficar preta (fading_out=True), depois clareia a cena NOVA a partir do
    # preto (fading_out=False),  daí o loop externo rodar duas vezes, uma pra cada source/direção.
    ...
    for source, fading_out in ((before_surface, True), (after_surface, False)):
        for i in range(steps):
            ...
            # progress vai de perto-de-0 até 1.0 ao longo dos `steps` frames. Se está escurecendo (fading_out), o preto começa
            # transparente e vai ficando opaco (alpha cresce); se tá clareando, é o contrário (alpha começa quase opaco e desce
            ...
            # redimensiona a tela virtual pro tamanho real da janela só na hora de mostrar, pra não precisar recriar a tela virtual toda vez que o jogador redimensionar a janela. Isso é feito no final do loop, depois de desenhar tudo na tela virtual (screen), e antes de chamar pygame.display.flip().
...
def run(screen, clock, character_image=None, character_name="Jogador", genero="m"):
    ...
    # Tela de introdução --> só aparece aqui na entrada da fase;
    ...
    # "bau": False) começam todos fechados/não coletados. collected também ganha uma entrada extra pra a 4ª engrenagem (a do papel), que
    ...
    # desenhado nem interativo, ver os `if not in_machine_room` mais abaixo).
    ...
    running = True
    while running:
        # dt = tempo (em segundos) desde o frame anterior, usado pra contar os timers (hint_timer, paper_gear_flash_timer) de forma
        # consistente, independente da velocidade real do computador.

        dt = clock.tick(60) / 1000

        # a janela real pode ter um tamanho diferente da tela virtual (width x height, o espaço em que todas as coordenadas da fase já
        # são calculadas) e converte mouse/clique de volta pra cá antes de checar qualquer colisão.
        ...
        # eventos: ESC e cliques do mouse (o movimento por WASD é
        # lido direto de pygame.key.get_pressed() mais abaixo, fora do loop de eventos, porque queremos saber se a tecla está SEGURADA,
                ...
                if not in_machine_room and ada_chat.tratar_clique_no_icone(click_pos):
                    ...
                    if not clicked_something and PAPER_RECT.collidepoint(click_pos):
                        ...
                        if all(collected.values()):
                            ...
                            # Ainda não coletou as 4 engrenagens e mostra
                            # um aviso por 2 segundos em vez de abrir o
                            # puzzle.
                            hint_msg = "Ainda faltam engrenagens para juntar."
                            hint_timer = 2.0

        #atualiza o estado: qual sala estamos e como o personagem se move
        # Cada sala tem seu próprio retângulo de chão andável, e só a oficina tem um obstáculo (a bancada) -- escolhe os limites certos
        ...
        if walking_to_machine:
            # caminhada automática até a máquina do tempo (ignora o teclado do jogador); usa o mesmo mover_para() da caminhada
            # automática, só que o alvo é fixo (a máquina) em vez de vir de um clique.
            ...
            # Movimento normal: lê o estado ATUAL do teclado (quais teclas estão seguradas agora, não só a que foi apertada nesse
            # frame) e deixa a classe Jogador decidir pra onde andar. Só acontece com a conversa com a Ada fechada -- com ela
            # aberta, o personagem fica parado (mesma regra do Gerbert).
            keys = pygame.key.get_pressed()
            jogador.mover(keys, floor_rect, table_rect)

        #desenho: fundo da sala atual, HUD (título) e o resto por cima
        ...
        if in_machine_room:
            # sala separada: só a máquina do tempo, os objetos da oficina (baú/pote/caixote/papel) já cumpriram seu papel e não existem
            # mais aqui.
            ...
            for c in CONTAINERS:
                ...
                # Se o objeto já está aberto mas a engrenagem de dentro ainda não foi coletada, desenha ela por cima com um
                # brilho dourado quando o mouse está perto (calculado pela distância, math.hypot, em vez de um retângulo, porque a
            ...
            #engrenagem escondida no desenho do papel (canto superior direito) é só uma área clicável sobre a arte de fundo; o
            # desenho em si nunca muda, só o brilho dourado (hover, e um flash rápido no momento da coleta).
            ...
            # papel / planta na parede — PAPER_RECT só existe para detecção de clique (collidepoint); nada é desenhado sobre a arte de fundo.
        ...
        # Dica fixa no rodapé muda de acordo com a sala: na oficina é sempre o HINT genérico (WASD/clique/ESC); na sala da máquina só
        # aparece depois que o personagem termina de andar sozinho até lá  pq antes disso, ficaria estranho já mandar "clique na máquina" com
