...
PONTO DE ENTRADA PARA O MENU GERAL (Pygame/menu/jogo.py): a classe
`Jogo` no final deste arquivo -- mesmo padrão de Fase_9/Fase_4/Fase_5.
Todo o resto do arquivo (janela, cenário, personagem, chatbot, puzzle)
continua rodando dentro da função `_executar_fase6`, chamada por
`Jogo.executar()`.
"""
# ==============================================================================
...
# ---------------------------------------------------------------------------
# Janela do jogo -- o tamanho é uma constante (usado tanto pela janela em si
# quanto por várias contas de escala/posição), mas a janela DE VERDADE só é
# criada dentro de _executar_fase6 (chamada por Jogo.executar()), não aqui
# no import do módulo -- assim o menu geral pode importar este arquivo sem
# abrir uma janela na hora, e cada vez que o jogador entra na fase ganha uma
# janela/estado novos (ver a classe Jogo mais abaixo).
# ---------------------------------------------------------------------------
LARGURA_JANELA = 960
ALTURA_JANELA = 600
...
# usados no jogo, cada um montado com caminho_asset(). Só guarda CAMINHOS
# (texto), então pode ficar aqui no nível do módulo sem depender da janela
# já existir -- o carregamento de verdade (pygame.image.load) só acontece
# dentro de _executar_fase6.
# ---------------------------------------------------------------------------
ASSETS = {
...
# "f" -> índice 1, ver _executar_fase6).
PASTAS_PERSONAGEM = [
...
# Fase_2/Fase_9/Fase_4/Fase_5 já usam: {"estrelas": 1-3, "completo": true,
# "tempo": "MM:SS"}. Esta fase NÃO tem cronômetro/limite de tempo (o
# puzzle do EDVAC é livre, sem pressa) -- por isso sempre dá 1 estrela ao
# concluir (ver _executar_fase6, no momento da vitória), em vez de calcular
...
def _salvar_progresso(estrelas, tempo_formatado):
    ...
    fase_5_atualizada.py) -- como esta fase só dá 1 estrela, na prática
    isso só evita sobrescrever o "tempo" de um recorde antigo com um pior."""
    import json
...
class Jogo:
    ...
    -- mesmo padrão de Fase_9/Fase_4/Fase_5: aceita character_image/
    character_name/genero vindos do menu (personagem escolhido lá
    continua o mesmo aqui). `executar()` só devolve "vitoria" quando o
    jogador conclui a fase (energiza o EDVAC e chega na porta); em
    qualquer outro jeito de sair (ESC, ou o painel de configurações),
    devolve None. Fechar a JANELA (evento QUIT) continua encerrando o
    programa INTEIRO, não só a fase -- mesmo comportamento de sempre
    desta fase, igual ao que já acontece em Pygame/Fase_4/fase4_final.py
    e Pygame/Fase_5/fase_5_atualizada.py."""
    ...
    def executar(self):
        return _executar_fase6(self.character_name, self.genero)


def _executar_fase6(character_name, genero):
    """Contém a fase inteira (janela, cenário, personagem, chatbot,
    menu de configurações e puzzle do EDVAC) -- é o antigo corpo deste
    arquivo (que antes rodava direto no import, sem função nenhuma),
    só que agora dentro de uma função pra poder ser chamada de novo a
    cada vez que o jogador entra na fase pelo mapa, sempre com estado
    limpo (nenhuma variável daqui é compartilhada entre uma chamada e
    outra)."""
    ...
    ticks_inicio_fase6 = pygame.time.get_ticks()

    # ---------------------------------------------------------------------------
    # Pasta do personagem escolhido no MENU GERAL (genero "m" -> índice 0,
    # "f" -> índice 1, mesma convenção das outras fases), e as duas imagens
    # de cenário (EDVAC desligado e ligado), redimensionadas para caber
    # exatamente na janela do jogo. fundo_atual guarda qual das duas está
    # sendo exibida — o jogo começa com a máquina desligada.
    # ---------------------------------------------------------------------------
    ...
    # devolve para o mesmo lugar. Reembaralhado aqui dentro (não mais no
    # nível do módulo), então cada nova visita à fase começa com uma ordem
    # diferente, em vez de reaproveitar sempre o mesmo sorteio da primeira
    # vez que o arquivo foi importado.
    ...
    # no final: "vitoria" (energizou o EDVAC e chegou na porta), None (saiu
    ...
    while rodando:
        ...
        for evento in pygame.event.get():
            ...
            # abaixo. Sem isso, um Esc que FECHA o puzzle (ou o menu) já
            # deixaria puzzle_edvac_aberto/menu_aberto em False a tempo do
            # próprio if de "sair da fase" (mais abaixo, no mesmo laço)
            # também disparar sobre o MESMO Esc, saindo da fase inteira
            # sem querer no clique que deveria só fechar aquela sub-tela.
            nenhuma_subtela_aberta_antes_do_evento = (
                not menu_aberto and not puzzle_edvac_aberto and not caixa_von_neumann_aberta
            )
            ...
            if evento.type == pygame.MOUSEBUTTONDOWN:
                if not menu_aberto and not puzzle_edvac_aberto and AREA_ENGRENAGEM.collidepoint(evento.pos):
                    ...
                    if BOTAO_PAUSA.collidepoint(evento.pos):
                        ...
                        # Fase_2/Fase_9 (config_fase2/config_fase9.
            ...
            # Fora de qualquer sub-tela (menu, puzzle, conversa), Esc volta
            # direto pro mapa de fases -- mesma convenção usada em todas as
            # outras fases já conectadas ao menu (Fase_2/Fase_9: "ESC volta
            # ao mapa de fases").
            if (
        ...
        # dele -- mesmo padrão visual de Fase_2/Fase_9 (_draw_player).
    ...
    # efeito ainda tocando, pra nada da Fase 6 vazar pro menu (mesmo motivo
    ...
    if vitoria_alcancada:
        ...
        tempo_gasto_segundos = (pygame.time.get_ticks() - ticks_inicio_fase6) / 1000
