...
Fase_6/Fase_10. Diferente das fases, esta cena não é clicável no mapa: o
menu a chama automaticamente logo depois que o jogador vence a Fase 10.
Todo o resto do arquivo (janela, cenário, personagem, transição, confetes)
continua rodando dentro da função `_executar_cena_final`, chamada por
`Jogo.executar()`.
"""
...
# VERDADE só é criada dentro de _executar_cena_final (chamada por
# Jogo.executar()), pra cada visita à cena ganhar uma janela/estado novos.
# ---------------------------------------------------------------------------
LARGURA_JANELA = 960
ALTURA_JANELA = 600
...
# faltando — isso é tratado pelos "carregadores seguros" logo abaixo. Só
# guarda CAMINHOS (texto), então fica bem aqui no nível do módulo sem
# depender da janela já existir -- o carregamento de verdade só acontece
# dentro de _executar_cena_final.
# ---------------------------------------------------------------------------
ASSETS = {
...
def _executar_cena_final(character_name, genero):
    """Roda a cena final inteira (janela, cenário, personagem, transição,
    confetes) e só retorna quando o jogador sai por Esc -- fechar a janela
    (QUIT) continua encerrando o programa inteiro, igual às outras fases.
    Devolve None ao sair por Esc (não há "vitória"/"derrota" aqui, é só
    uma cutscene linear que sempre termina do mesmo jeito)."""
    tela = pygame.display.set_mode((LARGURA_JANELA, ALTURA_JANELA))
    pygame.display.set_caption("Escape Room - Cena Final")
    relogio = pygame.time.Clock()

    # ---------------------------------------------------------------
    # Personagem escolhido no MENU GERAL, recebido via `genero` -- mesma
    # convenção usada em todas as outras fases já conectadas (genero "m"
    # -> pasta_personagem_1, "f" -> pasta_personagem_2).
    # ---------------------------------------------------------------
    ...
    # Fechar a JANELA (evento QUIT) encerra o programa inteiro, igual já
    # acontece nas outras fases (Fase_4/Fase_5/Fase_1/Fase_3/Fase_6/Fase_7/
    # Fase_8/Fase_10). Sair por Esc devolve controle ao menu geral (None)
...
class Jogo:
    """Ponto de entrada desta cena para o menu geral (Pygame/menu/jogo.py)
    -- mesmo padrão de Fase_9/Fase_4/Fase_5/Fase_1/Fase_6/Fase_7/Fase_8/
    Fase_10. Não é uma fase clicável no mapa: o menu chama Jogo().executar()
    automaticamente depois que o jogador vence a Fase 10."""
    ...
    def executar(self):
        return _executar_cena_final(self.character_name, self.genero)


if __name__ == "__main__":
    Jogo(genero="f").executar()
