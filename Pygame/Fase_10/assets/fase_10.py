...
# ---------------------------------------------------------------------------
# Janela do jogo (mesmo tamanho da Fase 1: 960x600)
# ---------------------------------------------------------------------------
LARGURA_JANELA = 960
ALTURA_JANELA = 600
...
# permite trocar manualmente qual personagem é carregado:
# 0 = primeiro personagem, 1 = segundo personagem (igual na Fase 1).
# ---------------------------------------------------------------------------
PERSONAGEM_ESCOLHIDO = 1

# ---------------------------------------------------------------------------
# ASSETS: dicionário central com TODOS os caminhos de imagem/fonte/som.
#
...
# >>> Ajuste estes valores se, na sua arte, o portal estiver em outro lugar. <<<
# Os valores são frações do tamanho da janela, então funcionam mesmo se a
# resolução da sua imagem de cenário for diferente.
# ---------------------------------------------------------------------------
...
# >>> Ajuste estes valores se, na sua arte, o PC estiver em outro lugar. <<<
# Os valores sao fracoes da janela, entao funcionam em qualquer resolucao.
# ---------------------------------------------------------------------------
AREA_PC = pygame.Rect(
...
# mesma ideia da bandeja da torre de protocolos, mais abaixo.
...
while rodando:
    ...
    # Verifica (a cada quadro) se o jogo já foi vencido: torre montada E
    # personagem encostou no portal. Usado para bloquear controles e mostrar
    # a tela final.
    fim_de_jogo = torre_completa and False  # recalculado mais abaixo, após mover
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN:
            ...
            if not menu_aberto and AREA_ENGRENAGEM.collidepoint(evento.pos):
                ...
                # Clicar em cima de uma peça "gruda" ela no cursor: guardamos
                # qual peça é e o deslocamento do clique dentro dela. O
                # encaixe de verdade só é conferido ao SOLTAR o botão do
                # mouse (evento MOUSEBUTTONUP, tratado mais abaixo).
                # -----------------------------------------------------------
                ...
                # Se for o pedaço certo da vez, ele entra na barra de
                # endereço. Se for errado, a URL "reseta" pra bandeja
                # embaralhada de novo e mostra a ordem certa como dica.
                # -----------------------------------------------------------
                ...
                # -----------------------------------------------------------
                # Botão que abre a torre de protocolos: só funciona depois que
                # o jogador já resolveu o puzzle da URL no computador. Antes
                # disso, avisa que é preciso usar o PC primeiro.
                if (
                    not torre_completa
                ...
                # Computador da esquerda: abre o puzzle da URL (só antes de
                # já estar resolvido).
                elif (
                    not url_resolvida
                    and not caixa_tim_aberta
                ...
                # Ícone do Tim: 1º clique começa a conversa; depois, abre a
        ...
        if evento.type == pygame.MOUSEMOTION and puzzle_aberto and peca_arrastando is not None:
            # Enquanto arrasta, a peça segue o cursor — respeitando o
            # deslocamento do clique original, pra não "pular" pro canto.
        ...
        if evento.type == pygame.MOUSEBUTTONUP and puzzle_aberto and peca_arrastando is not None:
            # Ao soltar: verifica se o CENTRO da peça caiu dentro do espaço
            # alvo atual (o próximo espaço a ser preenchido). A área de
            # encaixe é generosa (inflate) pra não exigir mira perfeita.
            ...
            # Se soltou FORA do alvo, a peça nunca chegou a sair da bandeja
            # (só estava "grudada" visualmente no cursor) — então ela volta
            # sozinha pro lugar, sem nenhuma penalidade. Isso evita punir
            # quem só errou a mira.
    ...
    # Aparece enquanto a torre não está pronta e nenhuma caixinha está aberta.
    # Fica destacado (mais claro) quando o mouse passa por cima.
    # -----------------------------------------------------------------------
    ...
    if not torre_completa and not puzzle_aberto and not url_aberta and not caixa_tim_aberta and not menu_aberto:
        if url_resolvida:
            ...
            # Botão apagado (cinza) enquanto a URL ainda não foi resolvida —
            # sinaliza que ele ainda não está liberado.
            cor_botao = (90, 90, 100)
    ...
    # Só aparece enquanto a URL ainda não foi resolvida e nenhuma
    # caixinha/painel/menu está aberto, igual ao brilho de outros clicáveis.
    # -----------------------------------------------------------------------
    if (
    ...
    # Depois da torre pronta, um brilho pulsa em volta do portal e um texto
    # convida o jogador a andar até lá.
    # -----------------------------------------------------------------------
    ...
    if puzzle_aberto:
        ...
        # Título e instrução curta e neutra — NÃO entrega a ordem certa nem
        # avisa quais peças são falsas; isso só o Tim ensina.
        ...
        # Numera os 4 espaços da torre de baixo pra cima (1 = base, 4 =
        # topo). Isso ensina a MECÂNICA (arrastar de baixo pra cima), não a
        # resposta — a ordem das peças continua sendo descoberta com o Tim.
        ...
        # Seta "comece pela base" apontando pro espaço 1 — some assim que a
        # primeira peça é encaixada, pra não poluir a tela depois.
        if proxima_peca == 0:
        ...
        # Centro da peça sendo arrastada, usado pra saber se ela está em
        # cima do espaço alvo (acende o alvo em verde nesse caso).
        centro_arrastando = None
        if peca_arrastando is not None:
            centro_arrastando = (arrasto_x + LARGURA_PECA // 2, arrasto_y + ALTURA_PECA // 2)

        # Espaços vazios que ainda faltam preencher. O PRÓXIMO espaço (o
        # alvo atual) fica destacado: pisca (mais claro) normalmente e
        # acende em VERDE quando a peça arrastada está em cima dele.
        ...
        # Peças disponíveis na bandeja (embaralhadas, certas e falsas). A
        # peça sendo arrastada NÃO é desenhada aqui — ela é desenhada
        # separadamente, por cima de tudo, seguindo o cursor (mais abaixo).
        ...
        # Mensagem de erro do puzzle (dentro do painel).
        ...
        # Peça sendo arrastada: desenhada por ÚLTIMO pra ficar por cima de
        # tudo (torre, espaços e bandeja), seguindo o cursor do mouse.
        if peca_arrastando is not None:
    ...
    if url_aberta:
        ...
        # Painel principal, estilo "tela de computador" (fundo escuro, borda clara).
        ...
        # Barra de endereço: vai mostrando os pedaços já colocados, em ordem.
        ...
        # Pedaços disponíveis na bandeja (embaralhados). Destaca o pedaço sob o mouse.
        ...
        # Depois do primeiro erro, mostra a ordem certa como dica.
        if erros_url >= 1:
            desenhar_texto_multilinha(
    ...
    if caixa_tim_aberta:
        ...
        # Avatar redondo grudado em cima da caixinha.
        ...
        # Escolhe qual fala mostrar de acordo com a etapa da conversa.
        if etapa_conversa_tim == ETAPA_APRESENTACAO:
        ...
        # Campo de digitação (só nas etapas em que o jogador escreve algo).
    ...
    if menu_aberto:
        ...
        # Painel de "madeira" desenhado (sem imagem).
    ...
    if chegou_no_portal:
        ...
        # Na tela final, Esc encerra o jogo.
